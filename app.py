# ================================================================
#   IT HELPDESK ANALYTICS — FINAL v14.0
#   WEBPAGE EXACT MATCH PDF — Charts + Tables + Arabic Names
...
.kpi{background:linear-gradient(145deg,#060f20,#0b1e3a);border:1px solid rgba(0,212,255,.12);border-top:3px solid #00d4ff;border-radius:20px;padding:22px 12px 18px;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,.6);transition:transform .3s,box-shadow .3s;margin-bottom:12px;animation:up .5s ease both}
...
.sec{background:linear-gradient(90deg,rgba(0,120,255,.1),transparent);border-left:3px solid #00d4ff;border-radius:0 12px 12px 0;padding:11px 22px;margin:28px 0 16px;color:#e0f0ff;font-size:1rem;font-weight:800;animation:fi .5s ease}
...
.stTabs [data-baseweb="tab"]{border-radius:12px;padding:9px 24px;font-size:.87rem;font-weight:700;color:#4a7a9a!important;background:transparent;transition:all .3s ease}
...
::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-track{background:#060f20}
...
T = {
    'AR': {
        ...
        'top_issue_lbl':'🔥 أكثر مشكلة تكراراً','coverage_pct':'📋 نسبة التغطية','accuracy_title':'✅ دقة البيانات',
    },
    'EN': {
        ...
        'tab_raw':'🗃️ Raw Data','kpi_sec':'📌 Key Performance Indicators','ai_insights':'🤖 AI Smart Insights',
        'top_agent_lbl':'🏆 Most Active Agent','top_dept_lbl':'🏅 Busiest Department',
        'top_issue_lbl':'🔥 Top Issue','coverage_pct':'📋 Agent Coverage','accuracy_title':'✅ Data Accuracy Verified',
    }
}
C_DEPT='إدارة العميل'; C_SVC='الخدمة'; C_MAIN='التصنيف الرئيسي'; C_SUB='التصنيف الفرعي'; C_AGENT='مسند الى'

# ── SIDEBAR ──────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<div style='text-align:center;padding:16px 0 8px'><div style='background:linear-gradient(135deg,#0038a0,#00aaff);display:inline-block;border-radius:16px;padding:12px 16px;font-size:2rem;box-shadow:0 4px 20px rgba(0,140,255,.4)'>🖥️</div></div>",unsafe_allow_html=True)
    lang = st.radio("🌐 Language", ["EN","AR"], horizontal=True)
    tx = T[lang]
    st.markdown(f"<h3 style='text-align:center;color:#00d4ff!important;margin:4px 0 12px;font-size:.92rem;font-weight:800'>{tx['title']}</h3>",unsafe_allow_html=True)
    st.markdown(f"<div style='text-align:center;font-size:.75rem;font-weight:700;color:{'#00dc78' if FONT_OK else '#ffc800'}'>{'✅ Arabic Font Ready' if FONT_OK else '⚠️ Font Loading...'}</div>",unsafe_allow_html=True)
    st.markdown("---")
    uploaded = st.file_uploader(tx['upload'], type=["xlsx","xls"])
    if uploaded: st.success(f"✅ {uploaded.name}")

if not uploaded:
    st.markdown(f"<div style='min-height:85vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px'><div style='background:linear-gradient(135deg,#0038a0,#00aaff);border-radius:28px;padding:22px;font-size:3.8rem;margin-bottom:26px;box-shadow:0 16px 50px rgba(0,150,255,.45)'>🖥️</div><h1 style='color:#00d4ff;font-size:2.8rem;font-weight:900;margin:0 0 14px'>{tx['title']}</h1><p style='color:#4a7a9a;font-size:1.05rem'>Upload your Excel file from the sidebar</p></div>",unsafe_allow_html=True)
    st.stop()
...
def load_data(rb):
    ...
    df=df.dropna(subset=mc,how='all'); df.reset_index(drop=True,inplace=True)
...
try:
    rb = uploaded.read(); df, acc = load_data(rb)
...
def pbar(label,val,mx,cnt,color="#00d4ff"):
    p=round(val/mx*100) if mx else 0; s=(label[:28]+'…') if len(label)>28 else label
...
def fig_to_png(fig, w=900, h=420):
    ...
    try:
        img_bytes = fig.to_image(format="png", width=w, height=h, scale=1.5)
        return img_bytes
    except Exception as e:
        return None

# ── PDF GENERATION — CHARTS + TABLES + ARABIC ───────────────────
def generate_pdf(df_data, stats):
    """Generate complete PDF with charts + tables matching webpage"""
    buffer = io.BytesIO()
    total  = len(df_data)
    W      = landscape(A4)[0] - 55   # usable width
    ...
    DARK  = colors.HexColor('#020810')
    NAVY  = colors.HexColor('#060f20')

    ts = ParagraphStyle('T',fontSize=22,textColor=BLUE,alignment=TA_CENTER,
                        fontName='Helvetica-Bold',spaceAfter=6)
    ss = ParagraphStyle('S',fontSize=10,textColor=colors.HexColor('#666'),
                        alignment=TA_CENTER, spaceAfter=12)
    h2 = ParagraphStyle('H2',fontSize=15,textColor=LBLUE,fontName='Helvetica-Bold',
                        spaceBefore=12,spaceAfter=8)
    h3 = ParagraphStyle('H3',fontSize=12,textColor=colors.HexColor('#333'),
                        fontName='Helvetica-Bold',spaceBefore=8,spaceAfter=6)
    ft = ParagraphStyle('FT',fontSize=7.5,textColor=colors.HexColor('#999'),
                        alignment=TA_CENTER)
    ar_norm = ParagraphStyle('AN',fontSize=9,fontName=AR_FONT,leading=13)

    def tbl(data, widths, hdr_color, bg1=None, bg2=None):
    ...
    def add_chart(fig, w_in=10, h_in=3.8):
        """Embed plotly chart as image in PDF"""
        png = fig_to_png(fig, int(w_in*96), int(h_in*96))
        if png:
            img_buf = io.BytesIO(png)
            story.append(Image(img_buf, width=w_in*inch, height=h_in*inch))
        else:
            story.append(Paragraph("[Chart could not be rendered]", ss))
    ...
    story.append(Paragraph(
        f"Generated: {datetime.now().strftime('%B %d, %Y  |  %I:%M %p')}  •  File: {uploaded.name}",
        ss))
    story.append(HRFlowable(width="100%", thickness=1.5,
                             color=LBLUE, spaceAfter=12))

    # KPI Table (matches webpage KPI cards)
    ...
    story.append(tbl(kpi_r, [3.2*inch, 2.2*inch, 1.8*inch], BLUE,
    ...
    story.append(tbl(hl_r, [2.5*inch, 3.8*inch, 1.2*inch, 1*inch], GREEN,
                     colors.white, colors.HexColor('#effffa')))
    story.append(PageBreak())

    # ══════════════════════════════════════════════════════════════
    # PAGE 2 — SERVICE + ISSUES CHARTS (Webpage Overview Tab)
    # ══════════════════════════════════════════════════════════════
    story.append(Paragraph("📊 OVERVIEW — SERVICE & ISSUE DISTRIBUTION", h2))

    # Build the same charts as webpage
    svc_data = dff[C_SVC].value_counts().reset_index()
    svc_data.columns = ['Service','Count']
    fig_svc = px.pie(svc_data, values='Count', names='Service',
    ...
    mc_data = dff[C_MAIN].value_counts().head(8).reset_index()
    mc_data.columns = ['Category','Count']
    fig_mc = px.pie(mc_data, values='Count', names='Category',
    ...
    # Side by side
    png_svc = fig_to_png(fig_svc, 560, 380)
    png_mc  = fig_to_png(fig_mc,  560, 380)

    if png_svc and png_mc:
        from reportlab.platypus import HRFlowable
        row = Table(
            [[Image(io.BytesIO(png_svc), width=4.8*inch, height=3.2*inch),
              Image(io.BytesIO(png_mc),  width=4.8*inch, height=3.2*inch)]],
            colWidths=[5*inch, 5*inch]
        )
        story.append(row)
    elif png_svc:
        story.append(Image(io.BytesIO(png_svc), width=9*inch, height=3.5*inch))
    ...
    fig_dv = px.bar(dv, x='Count', y='Dept', orientation='h', color='Count',
                    color_continuous_scale='Blues', template='plotly_white', text='Count',
    ...
    # Issues table (full list — top 20)
    story.append(Paragraph("📋 Top 20 Issue Categories — Complete Data", h3))
    i_rows = [['#','Issue Category','Count','% of Total','Priority']]
    for i,(name,cnt) in enumerate(_is.head(20).items(),1):
        pct=round(cnt/total*100,1)
        pri='High' if pct>8 else 'Med' if pct>3 else 'Low'
        i_rows.append([str(i), ar(name)[:52], f"{int(cnt):,}", f"{pct}%", pri])
    story.append(tbl(i_rows,[0.4*inch,4.2*inch,1*inch,0.9*inch,1*inch], RED,
                     colors.white, colors.HexColor('#fff5f5')))
    story.append(PageBreak())

    # ══════════════════════════════════════════════════════════════
    # PAGE 4 — DEPARTMENTS TABLE (FULL)
    # ══════════════════════════════════════════════════════════════
    story.append(Paragraph("🏢 DEPARTMENT PERFORMANCE — TOP 20", h2))
    
    d2 = dff[C_DEPT].value_counts().head(15).reset_index(); d2.columns=['Dept','Count']
    ...
    story.append(tbl(d_rows,[0.4*inch,4.2*inch,1*inch,0.9*inch,1*inch], LBLUE,
                     colors.white, colors.HexColor('#f0f8ff')))
    story.append(PageBreak())

    # ══════════════════════════════════════════════════════════════
    # PAGE 5 — AGENTS CHART + TABLE (FULL ARABIC NAMES)
    # ══════════════════════════════════════════════════════════════
    if not df_data[C_AGENT].dropna().empty:
        story.append(Paragraph("👨‍💻 AGENT PERFORMANCE — TOP 25 (FULL NAMES)", h2))
        ...
        # Full agent table with Arabic names (NO ■■■■■)
        story.append(Paragraph("📋 Top 25 Agents — Full Arabic Names", h3))
        ...
        story.append(tbl(a_rows,[0.4*inch,4.2*inch,1*inch,0.9*inch,1*inch], GREEN,
    ...
    story.append(Paragraph(
        f"IT Helpdesk Analytics Report  |  Generated: {datetime.now().strftime('%B %d, %Y %I:%M %p')}  |  100% Accurate  |  Made by Tarique Siddique",
        ft))
...
st.markdown(
    ...
    "<div style='background:linear-gradient(135deg,#0038a0,#0090ff);border-radius:18px;padding:16px;font-size:2.2rem;box-shadow:0 6px 24px rgba(0,140,255,.45)'>🖥️</div>"
    "<div style='flex:1'>"
    ...
    f"{badge}</div></div></div></div>",unsafe_allow_html=True)
...
with tab1:
    ...
    for col,(ico,val,lbl,clr) in zip([c1,c2,c3,c4,c5],[
        ...
        with col: st.markdown(f"<div class='metric-card'><div style='font-size:1.4rem'>{ico}</div><div style='font-size:1.6rem;font-weight:900;color:{clr};text-shadow:0 0 12px {clr}44;margin:6px 0'>{val}</div><div style='font-size:.65rem;color:#4a7a9a;font-weight:700;text-transform:uppercase'>{lbl}</div></div>",unsafe_allow_html=True)

    sec(tx['kpi_sec'])
    k1,k2,k3,k4,k5=st.columns(5)
    for col,(ico,val,lbl) in zip([k1,k2,k3,k4,k5],[
        ("🎫",len(dff),tx['total_rec']),("🏢",dff[C_DEPT].nunique(),tx['departments']),
        ("⚙️",dff[C_SVC].nunique(),tx['svc_types']),("🔥",dff[C_MAIN].nunique(),tx['issue_types']),
        ("👨‍💻",dff[C_AGENT].dropna().nunique(),tx['agents']),
    ]):
        with col: st.markdown(f"<div class='kpi'><span class='kpi-icon'>{ico}</span><span class='kpi-num'>{val:,}</span><span class='kpi-lbl'>{lbl}</span></div>",unsafe_allow_html=True)

    sec(tx['ai_insights'])
    a1,a2,a3=st.columns(3)
    with a1: st.markdown(f"<div class='ai-card'><div class='ai-badge'>🏢 {tx['top_dept_lbl']}</div><div class='ai-text'><b style='color:#00d4ff'>{td_name[:28]}</b> — <b>{td_cnt:,}</b> tickets</div></div>",unsafe_allow_html=True)
    with a2: st.markdown(f"<div class='ai-card'><div class='ai-badge'>🔥 {tx['top_issue_lbl']}</div><div class='ai-text'><b style='color:#ff6060'>{ti_name[:28]}</b> — <b>{ti_cnt:,}</b> tickets</div></div>",unsafe_allow_html=True)
    with a3: st.markdown(f"<div class='ai-card'><div class='ai-badge'>📋 {tx['coverage_pct']}</div><div class='ai-text'><b style='color:#40e0a0'>{cov}%</b> assigned</div></div>",unsafe_allow_html=True)

    i1,i2,i3,i4=st.columns(4)
    with i1: st.markdown(ins(tx['top_agent_lbl'],ta_name,f"{ta_cnt:,} tkts","#00d4ff"),unsafe_allow_html=True)
    with i2: st.markdown(ins(tx['top_dept_lbl'],td_name[:24],f"{td_cnt:,} tkts","#f0a020"),unsafe_allow_html=True)
    with i3: st.markdown(ins(tx['top_issue_lbl'],ti_name[:24],f"{ti_cnt:,} tkts","#ff4060"),unsafe_allow_html=True)
...
with tab5:
    ...
    with t1:
        ...
        td=dff[C_DEPT].value_counts().head(12); mx=int(td.iloc[0]) if len(td) else 1
        st.markdown(f"<div style='background:rgba(255,255,255,.02);border:1px solid rgba(0,212,255,.08);border-radius:16px;padding:18px 22px'>"+
                    "".join([pbar(str(n),int(v),mx,int(v)) for n,v in td.items()])+"</div>",unsafe_allow_html=True)
    with t2:
        st.markdown("<div style='color:#ff6060;font-weight:800;font-size:.9rem;margin-bottom:14px'>🔥 Issues</div>",unsafe_allow_html=True)
        ti=dff[C_MAIN].value_counts().head(12); mx2=int(ti.iloc[0]) if len(ti) else 1
        st.markdown(f"<div style='background:rgba(255,255,255,.02);border:1px solid rgba(0,212,255,.08);border-radius:16px;padding:18px 22px'>"+
                    "".join([pbar(str(n),int(v),mx2,int(v),"#ff4060") for n,v in ti.items()])+"</div>",unsafe_allow_html=True)
...
with tab6:
    ...
    if sr:
        ...
        sd=sd[mask]
    st.markdown(f"<div style='color:#4a7a9a;font-size:.83rem'><b style='color:#00d4ff'>{len(sd):,}</b> of <b>{len(df):,}</b> rows</div>",unsafe_allow_html=True)
...
with p1:
    st.markdown(
        ...
        f"<div class='ai-text'>"
        f"<b style='color:{'#40e0a0' if FONT_OK else '#ffc800'}'>{'✅ Arabic Font Ready' if FONT_OK else '⚠️ Font Loading'}</b><br>"
        f"✓ Page 1 — KPIs + Highlights (Webpage Match)<br>"
        f"✓ Page 2 — Service Pie + Issues Pie + Dept Bar Chart<br>"
        ...
        f"✓ All Arabic Names Correct (No ■■■■)"
        f"</div></div>",unsafe_allow_html=True)
with p2:
    st.markdown("<div style='text-align:center;padding:35px 0'><div style='font-size:5rem'>📥</div><div style='color:#e74c3c;font-size:1.1rem;font-weight:900;margin-top:14px'>5 PAGE</div><div style='color:#00d4ff;font-size:.95rem;font-weight:800'>FULL PDF</div></div>",unsafe_allow_html=True)
with p3:
    if st.button("📥 Generate Complete PDF",use_container_width=True,type="primary"):
        with st.spinner("🎨 Creating PDF with charts..."):
            try:
                ...
                st.success("✅ Done! PDF with all charts ready!")
                st.download_button(
                    "⬇️ DOWNLOAD COMPLETE REPORT (5 Pages)",
                    data=buf,
...
st.markdown("<div style='text-align:center;margin-top:40px;color:#4a7a9a;font-size:.88rem'>Made with 💙 by Tarique Siddique</div>",unsafe_allow_html=True)
